...
class ArrayAdapter {
	...
	update(finder_object, changed_data_object) {
		return Promise.resolve()
			.then(() => {
				let resultArr = [];
				let foundObject = utils.findingInArray(this.array_of_numbers, finder_object)
			...
			.then(() => {
				if (this.array_of_numbers.length != 0) {
				 Object.keys(deleting_object).map(item => {
						this.array_of_numbers = this.array_of_numbers.filter(element => element[item] != deleting_object[item])
						return []
					})
				} else {
					return [ ]
